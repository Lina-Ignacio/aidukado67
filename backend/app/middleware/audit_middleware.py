    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if token:
            try:
                ...
                if user_id_from_token:
                    user_id = int(user_id_from_token)
                    logger.warning(f"DEBUG middleware: setting user_id = {user_id}")
                    
            except JWTError as e:
                logger.warning(f"Invalid token: {e}")
        
        # This is outside the if token block — user_id is always defined above
        logger.warning(f"DEBUG middleware: final user_id = {user_id}")
        token_ctx = current_user_id.set(user_id)
