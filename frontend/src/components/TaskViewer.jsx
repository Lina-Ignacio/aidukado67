...
export default function TaskViewer({
  ...
  if (viewSubmittedFile && submissionData) {
    const fileExtension = submissionData.file_url?.split('.').pop();
    return (
      <div className="sm:w-3/5 sm:h-auto">
        <FileView
  ...
  return (
    <div className="w-full h-full flex flex-col items-center p-5">
      ...
      <div className="w-3/4 h-2/5 flex gap-5">
        ...
        <div className="child sm:w-3/4 sm:h-4/5 flex flex-col sm:min-h-[250px] rounded-md p-4 gap-5 bg-white">
          ...
          <p className="text-md text-[#F5C45E] font-bold">Due: October 10</p>
        ...
        <div className="child sm:w-1/4 sm:h-3/5 shadow-xl rounded-xl bg-[#F4F6FF] p-4 gap-5 border border-black/10">
          {userRole === "student" && (
            <div className="flex flex-col gap-4 w-full h-full items-start">
              {!submissionData ? (
                ...
                <>
                  ...
                  <p className="text-green-800 font-semibold">Score: 75/100</p>
                </>
              )}
            </div>
          )}
      ...
      <div className="child sm:w-3/4 shadow-md sm:h-auto rounded-lg p-4">
        <h2 className="text-[#102E50] font-bold text-xl">Remarks: </h2>
      </div>
    </div>
  );
}
